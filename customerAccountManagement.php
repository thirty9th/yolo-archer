        ...
        if($_SESSION["type"] == "customer"){
            ...
            if($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if($usernameIsValid && $passwordIsValid && $passwordsMatch && $nameIsValid && $addressIsValid && $ccNumberIsValid && $phoneIsValid && $emailIsValid){
                   ...
                   $sql = 'update customer set username="' . $_POST['username'] . '", password="' . $_POST['password1'] . '", name = "' . $_POST['name'] . '", address="' . $_POST["address"] .'", cc_number = ' . $_POST['cc_number'] . ', phone_number=' . $_POST['phone'] .',  email_address="' . $_POST['email'] . '" where id = ' . $row['id'];
            ...
            echo '<form action="customerAccountManagement.php" method="POST">';
            ...
            echo 'Credit Card Number: <input type="text" name="cc_number" value="' . $row["cc_number"] . '"/><br/>';
            if(!$ccNumberIsValid){echo ("Credit card number must be 16 numbers long.<br/>");}
            ...
            echo 'Email: <input type="email" name="email" value="' . $row["email_address"] . '"/><br/>';
            if(!$emailIsValid){echo ("Email must be longer than 3 characters and less than 45.<br/>");}
            ...
            echo "You are not a customer! You cannot manage your account here!";
        }
        
    }
    else{
        echo "You are not logged in. Please login before trying to manage your account.";
    }
